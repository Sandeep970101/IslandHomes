...
class _ApproveAddState extends State<ApproveAdd> {
  ...
  Future<void> approveAdd(String addId) async {
    ...
    try {
      final response = await http.put(Uri.parse('http://${NetworkUtils.ip}:3002/api/admin/approve-add/$addId'), headers: {
  ...
  Future<void> deleteAdd(String addId) async {
    try {
      final response = await http.delete(
        Uri.parse('http://${NetworkUtils.ip}:3002/api/admin/delete-add/$addId'),
        headers: {
          'Authorization': 'Bearer $token', // Attach the token to the request
          'Content-Type': 'application/json',
        },
