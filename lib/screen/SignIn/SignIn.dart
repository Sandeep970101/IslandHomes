...
class _SignInState extends State<SignIn> {
  ...
  Future<void> loginUser() async {
    ...
    const String apiUrl = 'http://${NetworkUtils.ip}:3002/api/user/login';
    ...
    if (response.statusCode == 200) {
          ...
          ? Navigator.of(context).pushReplacement(
              MaterialPageRoute(
                builder: (BuildContext context) => Home(
                  userId: jsonDecode(response.body)['user']
                      ['userId'], // Pass userId to AdminHome screen
                  token: jsonDecode(
                      response.body)['token'], // Pass token to AdminHome screen
                ),
              ),
            )
          : Navigator.of(context).pushReplacement(
      ...
      const SnackBar(content: Text('User Logged In Error.'));
